...
class TestSandboxConfigValidators:
    ...
    def test_extra_volumes_valid(self, tmp_path):
        host_path = "/data" if os.name != "nt" else str(tmp_path / "data")
        c = SandboxConfig(extra_volumes=[[host_path, "/data", "ro"]])
        assert c.extra_volumes == [[host_path, "/data", "ro"]]
