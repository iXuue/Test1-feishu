...
def _installed_pico_version() -> str:
    try:
        return importlib.metadata.version("pico-harness")
    except importlib.metadata.PackageNotFoundError:
        return importlib.metadata.version("codecub")
...
def test_default_session_info_contains_real_version(fake_agent_loop, config) -> None:
    ...
    info = _default_session_info(fake_agent_loop, config)
    expected_version = _installed_pico_version()
...
def test_default_session_info_falls_back_when_agent_loop_none(config) -> None:
    ...
    assert info["version"] == _installed_pico_version()
...
def test_resolve_context_window_helper_removed() -> None:
    ...
    session_py = Path(__file__).resolve().parents[2] / "pico" / "tui_rpc" / "methods" / "session.py"
