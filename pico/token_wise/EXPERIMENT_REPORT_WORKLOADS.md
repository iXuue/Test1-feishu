...
> `AgentLoop.run_turn`. Do not use these numbers as current release evidence.

_Two complementary workloads originally ran through the removed
``AgentLoop.process_direct`` path to test the claim that ``CacheOptimizer``
(4 breakpoints, history-aware) structurally outperforms the provider's built-in
single-breakpoint cache when history is large. The measurements have not been
rerun through current ``AgentLoop.run_turn``._
