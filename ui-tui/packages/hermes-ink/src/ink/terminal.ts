...
// Prefixes of XTVERSION-reported terminal names known to correctly implement
// the Kitty keyboard protocol or xterm modifyOtherKeys. Used to enable
// extended keys at runtime when env-sniffing fails — typically over SSH where
// TERM_PROGRAM / WT_SESSION are not forwarded, so detectTerminal() falls back
// to process.env.TERM (often 'xterm-256color') which is not on the env-sniff
// allowlist. XTVERSION reply travels through the pty and survives the SSH
// boundary, providing a reliable runtime probe.
//
// Match is case-insensitive prefix match against the trimmed reply payload
// (e.g. 'Zellij(4401)', 'xterm.js(5.5.0)', 'iTerm2 3.6', 'WezTerm 20240203').
const EXTENDED_KEYS_XTVERSION_PREFIXES = [
  'zellij',
  'tmux',
  'xterm.js',
  'ghostty',
...
/** True if the XTVERSION-reported terminal name is known to correctly handle
 *  extended key reporting. Used as a runtime fallback when env-based
 *  detection (supportsExtendedKeys) misses — notably over SSH where
 *  TERM_PROGRAM is stripped. Case-insensitive prefix match. */
export function isExtendedKeysCapableByXtversion(name: string): boolean {
  if (!name) {
    return false
  }
  ...
  return EXTENDED_KEYS_XTVERSION_PREFIXES.some(p => lc.startsWith(p))
}
