...
/**
 * Query the terminal and await responses behind a bounded barrier.
 *
...
export class TerminalQuerier {
  ...
  /**
   * Send the DA1 sentinel. Resolves when DA1's response arrives or the
   * barrier timeout expires.
   *
   ...
   * A timed-out barrier settles every pending item and disables this querier.
   * Without response identifiers, a late reply cannot be distinguished from a
   * later batch, so reusing the instance after a timeout would misroute data.
   */
  flush(timeoutMs = 500): Promise<void> {
    if (this.disabled) {
      return Promise.resolve()
    }

    return new Promise(resolve => {
      const sentinel: Extract<Pending, { kind: 'sentinel' }> = {
        kind: 'sentinel',
        resolve
      }

      sentinel.timeout = setTimeout(() => {
        this.disable()
      }, timeoutMs)

      this.queue.push(sentinel)
  ...
  private disable(): void {
    ...
    this.disabled = true
    this.settleThrough(this.queue.length - 1)
  }

  private settleThrough(index: number): void {
    if (index === -1) {
      return
    }

    for (const pending of this.queue.splice(0, index + 1)) {
