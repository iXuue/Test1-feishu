...
describe('readOsc52Clipboard', () => {
  ...
  it('bounds the whole read when both the query and flush remain pending', async () => {
    vi.useFakeTimers()

    const pending = new Promise<never>(() => undefined)
    const send = vi.fn().mockReturnValue(pending)
    const flush = vi.fn().mockReturnValue(pending)
    const read = readOsc52Clipboard({ flush, send }, 25)

    await vi.advanceTimersByTimeAsync(25)

    await expect(Promise.race([read, Promise.resolve('still pending')])).resolves.toBeNull()
